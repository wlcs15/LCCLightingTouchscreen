...
**Tag `v1.0.1` (pushed).** No lighting board on the bus 29-Aug-2026. **Do not flash A5.04** (that is LCCControlPanelTouchscreen, MAC `1c:db:d4:42:ef:d0`). Identify USB by `/dev/serial/by-id/`. RR-CirKits gateway `0483:5740` and Mega `2341:0042` — never flash. Sunday start: sister nodes A5.01–A5.04 were all up in LCC Pro with Configure; this tree is even with origin (nothing unpushed).
